...
// A new image URL lets preview services distinguish artwork revisions.
// Keep the stable aliases as well, so older HTML can still load its images.
const previews = {};
for (const lang of Object.keys(paths)) {
  const bytes = generated.get(`assets/social-preview-${lang}.png`);
  previews[lang] = hashName(`social-preview-${lang}`, bytes, "png");
  generated.set(previews[lang], bytes);
}
...
function documentFor(route, entry = false) {
  ...
  const preview = absolute(previews[route.lang]);
...
// Cached HTML can still request the previous release's CSS, JavaScript and previews.
...
for (const filename of previous) {
  if (
    ...
    /^assets\/[a-z][a-z0-9-]*-(?:[0-9a-f]{12}|[A-Z0-9]{8})\.(?:css|js|png)$/.test(filename)
  ) {
